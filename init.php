<?php
/*
Plugin Name: Slider 3D
Plugin URI: http://www.pluginswp.com/slider3d/
Description: Slider3d gallery is a flash real 3D with an expectacular desing.
Version: 1.2
...
function getYTidslider3d($ytURL) {
#
...
function slider3d_enqueue_scripts() { 
...
function slider3d($content){
	$content = preg_replace_callback("/\[slider3d ([^]]*)\/\]/i", "slider3d_render2", $content);
	return $content;
	
}

function slider3d_render2($tag_string){
	return slider3d_render($tag_string, "");
}

function slider3d_render($tag_string, $instance){
$contador=rand(9, 9999999);

$widthloading="48"; // Set if change loading image

global $wpdb; 	
$table_name = $wpdb->prefix . "slider3d";
...
*/
if($title!="")$nombrebox=$title;
$nombrebox=rand(99, 99999);
			...
			if($values!="") {
				...
				foreach ($items as &$value) {
					...
					if($item[2]!="" && $item[3]==1) {
						...
						$textovid.='&imagen'.($cont-1).'='.$item[2].'&title'.($cont-1).'='.$item[0].'&link'.($cont-1).'='.$item[6];
						$ligtext.= '<a href="'.$item[2].'" rel="shadowbox['.$nombrebox.']"></a>';			
						 $cont++;
						}
					}
...
if($round!="hide") $border=0;
	...
	$texto='cantidad='.$cantidad.'&columns='.$twidth.'&colorbordes='.str_replace("#", "", $color2).'&colortextos='.str_replace("#", "", $color3).'&speed2='.$op1.'&timeback='.$theight.'&controls='.$sizedescription.'&target='.$op2.'&onlink='.$sizetitle.'&speed='.$sizethumbnail.'&alpha='.$op4.'&type='.$number_thumbnails.'&border='.$border.'&menu3='.$font.'&color1=ffffff&menu5='.$op3.'&menu6=16&color2=000000&v1='.str_replace("#", "", $color1).'&v2='.$border.$textovid;
	
	$conta=0;



$urlpflash=plugins_url('slider3d.swf', __FILE__);

$output='

<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="'.$width.'" height="'.$height.'" id="slider3d'.$id.'-'.$contador.'" title="'.$title.'" align="middle">
...
function add_header_slider3d() {
$site_url = get_option( 'siteurl' );
 wp_enqueue_script('jquery');

}

class wp_slider3d extends WP_Widget {
	function wp_slider3d() {
		$widget_ops = array('classname' => 'wp_slider3d', 'description' => 'Slider3d gallery is a flash real 3D with an expectacular desing.' );
		$this->WP_Widget('wp_slider3d', 'SLIDER 3D', $widget_ops);
	}
...
if(empty($myrows)) {
	...
	echo '
	<p>First create a new gallery, from the administration of slider3d.</p>
	';
}
...
if(empty($re))
{
  ...
  $sql = " CREATE TABLE $table_name(
	...
	if($paca==1) $sql = "INSERT INTO $table_name (`title`, `width`, `height`, `border`, `round`, `width_thumbnail`, `height_thumbnail`, `thumbnail_border`, `thumbnail_round`, `number_thumbnails`, `ivalues`, `sizetitle`, `sizedescription`, `sizethumbnail`, `font`, `color1`, `color2`, `color3`, `time`, `animation`, `mode`, `op1`, `op2`, `op3`, `op4`, `op5`) VALUES ('', '100%', '300', '10', 'hide', '8', '5', '14', '', '0', 'Title video t6r4ndt6r4ndt6r4nd1t6r4ndhttp://www.youtube.com/watch?v=kBtMJjLVB90t6r4ndt6r4ndt6r4ndt6r4nd7/3/2012kh6gfd57hgg', '0', '0', '1', '0', '', 'FFFFFF', '000000', '', '', '', '0', '', 'Verdana', '90', '');";
	
	else $sql = "INSERT INTO $table_name (`title`, `width`, `height`, `border`, `round`, `width_thumbnail`, `height_thumbnail`, `thumbnail_border`, `thumbnail_round`, `number_thumbnails`, `ivalues`, `sizetitle`, `sizedescription`, `sizethumbnail`, `font`, `color1`, `color2`, `color3`, `time`, `animation`, `mode`, `op1`, `op2`, `op3`, `op4`, `op5`) VALUES ('', '100%', '300', '10', 'hide', '8', '5', '14', '', '0', 'Title video t6r4ndt6r4ndt6r4nd1t6r4ndhttp://www.youtube.com/watch?v=kBtMJjLVB90t6r4ndt6r4ndt6r4ndt6r4nd7/3/2012kh6gfd57hgg', '0', '0', '1', '0', '', 'FFFFFF', '000000', '', '', '', '0', '', 'Verdana', '90', '');";
...
usort($sorter, 'sortByOrder');
    ...
    $cont = $value['cont'];
		...
		if($_POST['operation']=="1") {
			$values.="Titlet6r4nd".""."t6r4nd".""."t6r4nd"."1"."t6r4nd".""."t6r4nd".""."t6r4nd".""."t6r4nd".""."t6r4ndkh6gfd57hgg";
...
function slider3d_add_menu(){	
	if (function_exists('add_options_page')) {
		//add_menu_page
		add_menu_page('slider3d', 'SLIDER 3D', 8, 'slider3d', 'slider3d_panel');
	}
}


if (function_exists('add_action')) {
	add_action('admin_menu', 'slider3d_add_menu'); 
}

add_action( 'widgets_init', create_function('', 'return register_widget("wp_slider3d");') );
add_action('init', 'add_header_slider3d');
add_filter('the_content', 'slider3d');
add_action('admin_head', 'slider3d_enqueue_scripts');
?>
